...
(function () {
  ...
  // Endpoints come from an injected config when running behind the
  // instrumenting proxy (window.__CF_CONFIG); otherwise the static-server
  // defaults apply. Keep these three lines together on consecutive lines —
  // test-feedback-config.js extracts this exact block.
  const CF_CONFIG = (typeof window !== "undefined" && window.__CF_CONFIG) || {};
  const HISTORY_URL = CF_CONFIG.historyUrl || "feedback/history.json";
  const FEEDBACK_URL = CF_CONFIG.feedbackUrl || "/feedback";
        ...
        banner.innerHTML = `
          <div class="cf-processing-row">
            ...
            <div class="cf-processing-body">
              <strong>Still waiting on a response…</strong>
              <span class="cf-processing-meta">${n} comment${n === 1 ? "" : "s"} · submitted ${submittedAgo}</span>
            </div>
          </div>
          <div class="cf-processing-status">
            No acknowledgment in <code>history.json</code> yet. If an agent is on this, it may simply be busy — reading code, editing, and running checks can take a few minutes; keep waiting. If nothing changes, make sure a Claude Code session is watching this directory: open a terminal here, run <code>claude</code>, and ask it to <em>"process pending feedback in this directory"</em>.
          </div>
  ...
  function onHistoryUpdated() {
    ...
    // Only changes that explicitly declare an `anchor` are expected to have a
    // DOM region. Lite-mode changes (no anchor) edit source instead of
    // annotating the page, so they're never "missing".
    const missing = trulyNew.filter(ch => ch.anchor && !findAnchorNode(ch.anchor));
    if (missing.length > 0) {
      const missingIds = missing.map(ch => ch.anchor).sort().join("|");
  ...
  function renderHistory() {
    ...
    $("cf-tour").disabled = flattenChanges().filter((c) => c.anchor).length === 0;
  }

  function focusChange(ch) {
    // Lite-mode changes carry no `anchor`: the agent edited source rather than
    // annotating the DOM, so there's no on-page region to highlight. Skip
    // silently instead of warning.
    if (!ch.anchor) return;
    const node = findAnchorNode(ch.anchor);
    if (!node) {
      showToast(`couldn't find region for change "${ch.anchor}"`, 3500);
      return;
    }
  ...
  function startTour() {
    // Only changes with an explicit `anchor` can be toured (Lite-mode changes
    // have no on-page region to walk through).
    const all = flattenChanges().filter((c) => c.anchor);
    if (!all.length) return;
    let startIdx = 0;
    if (history.length > 0) {
      // Find the last batch that has an anchored change
      for (let i = history.length - 1; i >= 0; i--) {
        const firstAnchored = (history[i].changes || []).find((c) => c.anchor);
        if (firstAnchored) {
          const idx = all.findIndex(c => c.id === firstAnchored.id);
